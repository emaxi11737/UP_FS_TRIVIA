...
export default class RandomQuestionUseCase implements IRandomQuestionUseCase {
    ...
    public async random(randomQuestionFilterDto: IRandomQuestionFilterDto): Promise<IQuestionDto[]> {
        ...
        let questionFilter;
        if (!randomQuestionFilterDto.questionCategoriesId || randomQuestionFilterDto.questionCategoriesId.length === 0) throw Error("Question categories not found");
        ...
        questionFilter = new RandomQuestionFilter(randomQuestionFilterDto.questionCategoriesId, randomQuestionFilterDto.level, sizeDefault);
        const errors = await validate(questionFilter);
        if (errors.length > 0) throw Error("Please, check input params");

        return this.questionRepository.random(questionFilter);
    }
}
